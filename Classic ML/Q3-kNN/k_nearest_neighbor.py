...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.sqrt(X_sq[:, np.newaxis] + X_train_sq[np.newaxis, :] - 2 * np.dot(X, self.X_train.T))
        return dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            closest_y = self.y_train[sorted_indices[:min(k, self.y_train.shape[0])]]
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            #########################################################################
            # TODO:                                                                 #
            ...
            unique_closests, counts = np.unique(closest_y, return_counts=True)
            y_pred[i] = unique_closests[np.argmax(counts)]
